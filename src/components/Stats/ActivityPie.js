...
export default function ActivityPie(props) {
  ...
  const onlyHappy = props.moods.reduce((arr, mood) => {
    if (mood.mood === 'happy') {
      arr.push(mood.activity);
    }
    return arr;
  }, []);

  const getAmount = (weather) => {
    let regex = new RegExp(weather, "g");
    return onlyHappy.toString().match(regex) ? onlyHappy.toString().match(regex).length : 0;
  }
 
  const allActivities = ['low', 'medium', 'high'];

  const happyData = allActivities.reduce((arr, word) => {
    arr.push({
      "name": word,
